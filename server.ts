...
restoreOverwrittenFilesWithOriginals().then(() => {
  ...
  app.use('/ftp', security.isAuthorized(), serveIndexMiddleware, serveIndex('ftp', { icons: true })) // vuln-code-snippet vuln-line directoryListingChallenge
  ...
  app.use('/support/logs/:file', security.isAuthorized(), serveLogFiles()) // vuln-code-snippet vuln-line accessLogDisclosureChallenge
  ...
  app.use('/rest/user/reset-password', rateLimit({
    windowMs: 5 * 60 * 1000,
    max: 100,
    keyGenerator ({ ip }: { headers: any, ip: any }) { return ip } // Use only IP address, not spoofable headers
  }))
